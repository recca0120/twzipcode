...
class Twzipcode
{
    ...
    private static $newDistrictName = [
        '萬里鄉'  => '萬里區',
        '金山鄉'  => '金山區',
        '板橋市'  => '板橋區',
        '汐止市'  => '汐止區',
    ...
    {
        static::$twzipcodeData = static::loadTwzipcodeData();
    ...
    private function parse($address)
    {
        ...
        if (preg_match('/'.implode('|', array_keys(static::$twzipcodeData)).'/', $address, $countyMatch)) {
            $county = $countyMatch[0];
            $districts = static::$twzipcodeData[$county];
    ...
    public static function loadTwzipcodeData()
    {
    ...
    private function normalizeAddress($address)
    {
        ...
        $address = $this->normalizeStreet($address);
    ...
    {
        return preg_replace_callback('/^('.implode('|', array_keys(static::$newCountyName)).')/', function ($m) {
            return static::$newCountyName[$m[1]];
        }, $address);
    }

    private function normalizeDistrict($address)
    {
        return strtr($address, static::$newDistrictName);
    }

    public function normalizeStreet($address)
    {
        $counties = [
            '台中市',
            '台南市',
        ...
        if (preg_match('/'.implode('|', $counties).'/', $address) != false) {
            return strtr($address, static::$newStreetName);
        }
